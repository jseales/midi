...
class Event(object):
    ...
    def adjust_msdelay(self, tempo):
        ...
        self.msdelay = int(round((rtick * tempo.mpt) + tempo.msdelay))
    ...
    def __str__(self):
        return "%s [ %s ]" % \
                            (super(ProgramChangeEvent, self).__str__(),
                                constants.GM_INSTRUMENT_DICTIONARY[self.value])
...
def single_note_tuning_event(note_changes, device_number=None, tuning_program=None):
    ...
    ev.changes = [(key,semitone,int(round(float(cents)/100*(2**14)))) for (key,semitone,cents) in note_changes]
    return ev
...
class SetTempoEvent(MetaEvent):
    ...
    def __setattr__(self, item, value):
        if item == 'mpqn':
            ...
            self.__dict__['mpqn'] = int(round(float(6e7) / value))
        else:
    ...
    def encode_data(self):
        self.mpqn = int(round(float(6e7) / self.tempo))
        ...
        for event in self:
            if last:
                event.msdelay = last.msdelay + \
                    int(round(last.mpt * (event.tick - last.tick)))
            last = event
...
class EventStreamIterator(object):
    ...
    def __next_edge(self):
        ...
        self.window_edge += int(round(self.window_length / self.tempo.mpt))
...
class EventStream(object):
    ...
    def __set_resolution(self, resolution):
        ...
        for value in BEATVALUES:
            self.beatmap.append(int(round(value * resolution)))
    ...
    def clean_midi(self):
        '''Unimplemented. Will check for redundant messages and delete them
            ...
            control changes that are followed by a "Reset Controllers"
...
class EventStreamWriter(object):
    ...
    def __init__(self, midistream, output):
        try:
            ...
            "Path to output midifile is not string or unicode."
        self.output = output
